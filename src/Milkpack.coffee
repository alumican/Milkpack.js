...
# /hoge/fuga から /hoge に遷移したとき正しく処理されない
...
jpp.util.Scope.temp () ->
	...
	class Milkpack extends jpp.event.EventDispatcher
		...
		constructor: (option) ->
			...
			jpp.milkpack.Util.LOGGING = !@_isReleaseMode
			jpp.milkpack.Util.printInit(Milkpack, @_kazitori, @routes)

			# ユーザによる初期化
			@_initCommand = new jpp.command.Serial()
			@onInit()
		...
		addCommand: (commands...) ->
			@_initCommand.addCommand(commands...) if @_initCommand isnt null

		# 実行中のコマンドに新たなコマンドを挿入する
		insertCommand: (commands...) ->
			@_initCommand.insertCommand(commands...) if @_initCommand isnt null

		# 現在のフラグメントを取得する
		getTargetFragment: () -> return @_kazitori.fragment

		# 目的地のシーンを取得する
		getTargetScene: () -> return @_taregtScene
		...
		_kazitoriExecutedHandler: (event) =>
			@_log("kazitoriExecutedHandler : isInGeneratingSubFragment = #{@_isInGeneratingSubFragment}")
			@_log("kazitoriExecutedHandler : next                      = #{event.next}")
		...
		_pushRoute: (fragment) ->
			...
			# 対処療法
			if @_queue.length > 0 && @_queue[@_queue.length - 1] == route[0]
		...
		_nextAsync: () ->
			...
			#console.log(fragment, direction, @_targetFragment)

			if direction < 0 && fragment != @_targetFragment
				@_currentScene.addEventListener(jpp.milkpack.SceneEvent.CHANGE_STATUS, @_sceneChangeStatusHandler)
				@_currentScene.bye()
			else if direction >= 0
				@_currentScene.addEventListener(jpp.milkpack.SceneEvent.CHANGE_STATUS, @_sceneChangeStatusHandler)
